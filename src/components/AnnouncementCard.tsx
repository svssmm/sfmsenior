...
export default function AnnouncementCard({ item }: { item: any }) {
  return (
    <motion.div 
      ...
      viewport={{ once: true }}
      className="group p-6 rounded-2xl border border-gray-100 hover:border-sfm-mid/30 hover:shadow-xl hover:shadow-sfm-mid/5 transition-all bg-slate-50/50 cursor-default"
    >
      <div className="flex justify-between items-start mb-4">
        <h3 className="text-xl font-bold text-sfm-dark">{item.title}</h3>
